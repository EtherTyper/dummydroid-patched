...
final class FieldSet {
   ...
   public FieldSet clone() {
      FieldSet clone = newFieldSet();

      Entry entry;
      for(int i = 0; i < this.fields.getNumArrayEntries(); ++i) {
         entry = this.fields.getArrayEntryAt(i);
         clone.setField((FieldSet.FieldDescriptorLite)entry.getKey(), entry.getValue());
      }
   ...
   public Map getAllFields() {
      if (this.hasLazyField) {
         SmallSortedMap result = cloneAllFieldsMap(this.fields, false);
   ...
   private static void cloneFieldEntry(Map map, Entry entry, boolean copyList) {
      FieldSet.FieldDescriptorLite key = (FieldSet.FieldDescriptorLite)entry.getKey();
   ...
   public boolean hasField(FieldSet.FieldDescriptorLite descriptor) {
   ...
   public Object getField(FieldSet.FieldDescriptorLite descriptor) {
      Object o = this.fields.get(descriptor);
      return o instanceof LazyField ? ((LazyField)o).getValue() : o;
   }

   public void setField(FieldSet.FieldDescriptorLite descriptor, Object value) {
      if (descriptor.isRepeated()) {
   ...
   public void clearField(FieldSet.FieldDescriptorLite descriptor) {
   ...
   public int getRepeatedFieldCount(FieldSet.FieldDescriptorLite descriptor) {
      if (!descriptor.isRepeated()) {
   ...
   public Object getRepeatedField(FieldSet.FieldDescriptorLite descriptor, int index) {
      if (!descriptor.isRepeated()) {
         throw new IllegalArgumentException("getRepeatedField() can only be called on repeated fields.");
      } else {
         Object value = this.getField(descriptor);
   ...
   public void setRepeatedField(FieldSet.FieldDescriptorLite descriptor, int index, Object value) {
      if (!descriptor.isRepeated()) {
         throw new IllegalArgumentException("getRepeatedField() can only be called on repeated fields.");
      } else {
   ...
   public void addRepeatedField(FieldSet.FieldDescriptorLite descriptor, Object value) {
      if (!descriptor.isRepeated()) {
   ...
   private static boolean isInitialized(Entry entry) {
      FieldSet.FieldDescriptorLite descriptor = (FieldSet.FieldDescriptorLite)entry.getKey();
   ...
   private void writeMessageSetTo(Entry entry, CodedOutputStream output) throws IOException {
      FieldSet.FieldDescriptorLite descriptor = (FieldSet.FieldDescriptorLite)entry.getKey();
   ...
   public int getSerializedSize() {
      ...
      for(Iterator var4 = this.fields.getOverflowEntries().iterator(); var4.hasNext(); size += computeFieldSize((FieldSet.FieldDescriptorLite)entry.getKey(), entry.getValue())) {
         entry = (Entry)var4.next();
      }
   ...
   static final class Builder {
      ...
      private static void replaceBuilders(SmallSortedMap fieldMap) {
      ...
      private static void replaceBuilders(Entry entry) {
         entry.setValue(replaceBuilders((FieldSet.FieldDescriptorLite)entry.getKey(), entry.getValue()));
      }

      private static Object replaceBuilders(FieldSet.FieldDescriptorLite descriptor, Object value) {
         if (value == null) {
            return value;
         } else if (descriptor.getLiteJavaType() == WireFormat.JavaType.MESSAGE) {
            if (descriptor.isRepeated()) {
      ...
      public static FieldSet.Builder fromFieldSet(FieldSet fieldSet) {
         FieldSet.Builder builder = new FieldSet.Builder(FieldSet.cloneAllFieldsMap(fieldSet.fields, true));
      ...
      Object getFieldAllowBuilders(FieldSet.FieldDescriptorLite descriptor) {
         Object o = this.fields.get(descriptor);
         return o instanceof LazyField ? ((LazyField)o).getValue() : o;
      }
      ...
      Object getRepeatedFieldAllowBuilders(FieldSet.FieldDescriptorLite descriptor, int index) {
         if (!descriptor.isRepeated()) {
            throw new IllegalArgumentException("getRepeatedField() can only be called on repeated fields.");
         } else {
            Object value = this.getFieldAllowBuilders(descriptor);
