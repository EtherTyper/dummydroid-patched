...
public abstract class CodedInputStream {
   ...
   public static CodedInputStream newInstance(Iterable input) {
   ...
   static CodedInputStream newInstance(Iterable bufs, boolean bufferIsImmutable) {
      int flag = 0;
      int totalSize = 0;
      ...
      if (flag == 2) {
         return new CodedInputStream.IterableDirectByteBufferDecoder(bufs, totalSize, bufferIsImmutable);
      } else {
   ...
   private static final class IterableDirectByteBufferDecoder extends CodedInputStream {
      ...
      public boolean skipField(int tag, CodedOutputStream output) throws IOException {
         ...
         case 3:
            ...
            value = WireFormat.makeTag(WireFormat.getTagFieldNumber(tag), 4);
            this.checkLastTagWas(value);
      ...
      public MessageLite readGroup(int fieldNumber, Parser parser, ExtensionRegistryLite extensionRegistry) throws IOException {
         if (this.recursionDepth >= this.recursionLimit) {
            throw InvalidProtocolBufferException.recursionLimitExceeded();
         } else {
            ++this.recursionDepth;
            MessageLite result = (MessageLite)parser.parsePartialFrom((CodedInputStream)this, extensionRegistry);
            this.checkLastTagWas(WireFormat.makeTag(fieldNumber, 4));
            --this.recursionDepth;
            return result;
         }
      ...
      public MessageLite readMessage(Parser parser, ExtensionRegistryLite extensionRegistry) throws IOException {
         int length = this.readRawVarint32();
         if (this.recursionDepth >= this.recursionLimit) {
            throw InvalidProtocolBufferException.recursionLimitExceeded();
         } else {
   ...
   private static final class StreamDecoder extends CodedInputStream {
      ...
      private byte[] readRawBytesSlowPath(int size, boolean ensureNoLeakedReferences) throws IOException {
         ...
         if (result != null) {
            ...
            List chunks = this.readRawBytesSlowPathRemainingChunks(sizeLeft);
            byte[] bytes = new byte[size];
