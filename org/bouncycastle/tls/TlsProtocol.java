...
public abstract class TlsProtocol {
   ...
   protected TlsProtocol(InputStream var1, OutputStream var2) {
      this.blocking = true;
   ...
   protected void applyMaxFragmentLengthExtension() throws IOException {
      short var1 = this.securityParameters.getMaxFragmentLength();
      if (var1 >= 0) {
         ...
         int var2 = 1 << 8 + var1;
         this.recordStream.setPlaintextLimit(var2);
      }
   ...
   protected void completeHandshake() throws IOException {
      try {
         ...
         this.getContextAdmin().setSession(this.tlsSession);
   ...
   public void writeApplicationData(byte[] var1, int var2, int var3) throws IOException {
      if (this.closed) {
         if (this.failedWithError) {
            throw new IOException("Internal TLS error, this could be an attack");
         } else {
      ...
      if (this.expected_verify_data == null) {
         ...
         if (!Arrays.constantTimeAreEqual(this.expected_verify_data, var2)) {
            ...
            if (null == this.securityParameters.getTLSUnique()) {
               this.securityParameters.tlsUnique = var2;
            }
   ...
   protected void sendFinishedMessage() throws IOException {
      ...
      if (null == this.securityParameters.getTLSUnique()) {
         this.securityParameters.tlsUnique = var1;
      }
   ...
   protected byte[] createVerifyData(boolean var1) {
      TlsContext var2 = this.getContext();
      TlsHandshakeHash var3 = this.recordStream.getHandshakeHash();
      if (TlsUtils.isSSL(var2)) {
         TlsHash var4 = var3.forkPRFHash();
         byte[] var5 = var1 ? TlsUtils.SSL_SERVER : TlsUtils.SSL_CLIENT;
         var4.update(var5, 0, var5.length);
         return var4.calculateHash();
      } else {
         return TlsUtils.calculateTLSVerifyData(var2, var3, var1);
      }
   }
   ...
   protected static byte[] createRandomBlock(boolean var0, TlsContext var1) {
      byte[] var2 = var1.getCrypto().createNonce(32);
      if (var0) {
   ...
   protected static void establishMasterSecret(TlsContext var0, TlsKeyExchange var1) throws IOException {
      TlsSecret var2 = var1.generatePreMasterSecret();
      if (var2 == null) {
         throw new TlsFatalAlert((short)80);
      } else {
         try {
   ...
   protected static TlsCredentials validateCredentials(TlsCredentials var0) throws IOException {
      if (var0 != null) {
         byte var1 = 0;
         int var2 = var1 + (var0 instanceof TlsCredentialedAgreement ? 1 : 0);
         var2 += var0 instanceof TlsCredentialedDecryptor ? 1 : 0;
         var2 += var0 instanceof TlsCredentialedSigner ? 1 : 0;
         if (var2 != 1) {
            throw new TlsFatalAlert((short)80);
         }
      }
